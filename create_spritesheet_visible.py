...
def create_spritesheet_visible(image, singleRow):

    # Grab all visible layers from the original image, each one of which will become an animation frame
    layers = []
    ...
    for y in xrange(0, numRows):
        for x in xrange(0, numCols):
            ...
            if( not layers[layerIndex].visible):
                continue
...
register(
    "python_fu_create_spritesheetvisible",
    "Creates a spritesheet (in a new image) from the visible layers of the current image.",
    "Creates a spritesheet (in a new image) from the visible layers of the current image.",
    "Balazs Papp",
    "Balazs Papp",
    "2019",
    "Create Spritesheet from visbile layers",
    "*",
    [
    ...
    [],
    create_spritesheet_visible, menu="<Image>/Filters/Animation/")
